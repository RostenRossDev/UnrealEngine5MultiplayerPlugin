	...
	JoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnJoinSessionComplete))
{
	...
	if (OnlineSubsystem) {
		...
		FString name = *OnlineSubsystem->GetSubsystemName().ToString();
		FString msg = "Found subsystem " + name;
		if (GEngine) {
	...
	if (bWasSuccessful) 
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Green,
				FString::Printf(TEXT("Sesión %s creada con exito"), *SessionName.ToString())
			);
		}
	...
	else
	{
		if(GEngine)
		{
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Green,
				FString::Printf(TEXT("Error al crear la session:  %s"), *SessionName.ToString())
			);
		}
	}
}
		...
		if (MatchType == FString("FreeForAll")) {
			...
			if (GEngine) {
				GEngine->AddOnScreenDebugMessage(
					...
					15.f,
					FColor::Green,
					FString::Printf(TEXT("Sesion encontrada de: %s"), *MatchType)
				);
			}
	...
	if (OnlineSessionInterface->GetResolvedConnectString(NAME_GameSession, Address)) {
		if (GEngine) {
			GEngine->AddOnScreenDebugMessage(
				...
				15.f,
				FColor::Yellow,
				FString::Printf(TEXT("Conexión: %s"), *Address)
			);
		}
